...
// 'MySampleOption' in this example.
type ServiceConfig struct {
	MySampleOption MySampleOptionConfig
}

// MySampleOptionConfig is example of service's custom structured configuration that is specified in the service's
// configuration.yaml file and Configuration Provider (aka Consul), if enabled.
type MySampleOptionConfig struct {
	OffImageLocation string
	OnImageLocation  string
	Writable         MySampleWritable
}

// MySampleWritable defines the service's custom configuration writable section, i.e. can be updated from Consul
type MySampleWritable struct {
...
func (scc *MySampleOptionConfig) Validate() error {
	if len(scc.OnImageLocation) == 0 {
		return errors.New("MySampleOption.OnImageLocation configuration setting can not be blank")
	}

	if len(scc.OffImageLocation) == 0 {
		return errors.New("MySampleOption.OffImageLocation configuration setting can not be blank")
	}

	if scc.Writable.DiscoverSleepDurationSecs < 10 {
		return errors.New("MySampleOption.Writable.DiscoverSleepDurationSecs configuration setting must be 10 or greater")
	}
