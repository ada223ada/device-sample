...
// Package driver provides a mysample example implementation of
...
import (
	...
	"github.com/edgexfoundry/device-mysample/config"
...
// service.
func (s *MySampleDriver) Initialize(sdk interfaces.DeviceServiceSDK) error {
	s.sdk = sdk
	...
	if err := sdk.LoadCustomConfig(s.serviceConfig, "MySampleOption"); err != nil {
		return fmt.Errorf("unable to load 'MySampleOption' custom configuration: %s", err.Error())
	}

	s.lc.Infof("Custom config is: %v", s.serviceConfig.MySampleOption)

	if err := s.serviceConfig.MySampleOption.Validate(); err != nil {
		return fmt.Errorf("'MySampleOption' custom configuration validation failed: %s", err.Error())
	}

	if err := sdk.ListenForCustomConfigChanges(
		&s.serviceConfig.MySampleOption.Writable,
		"MySampleOption/Writable", s.ProcessCustomConfigChanges); err != nil {
		return fmt.Errorf("unable to listen for changes for 'MySampleOption.Writable' custom configuration: %s", err.Error())
	}
...
func (s *MySampleDriver) ProcessCustomConfigChanges(rawWritableConfig interface{}) {
	updated, ok := rawWritableConfig.(*config.MySampleWritable)
	if !ok {
		s.lc.Error("unable to process custom config updates: Can not cast raw config to type 'MySampleWritable'")
		return
	}

	s.lc.Info("Received configuration updates for 'MySampleOption.Writable' section")

	previous := s.serviceConfig.MySampleOption.Writable
	s.serviceConfig.MySampleOption.Writable = *updated
...
func (s *MySampleDriver) HandleReadCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []sdkModels.CommandRequest) (res []*sdkModels.CommandValue, err error) {
	s.lc.Debugf("MySampleDriver.HandleReadCommands: protocols: %v resource: %v attributes: %v", protocols, reqs[0].DeviceResourceName, reqs[0].Attributes)
	...
	for _, req := range reqs {
		...
		case "Image":
			...
			if s.switchButton {
				err = getImageBytes(s.serviceConfig.MySampleOption.OnImageLocation, buf)
			} else {
				err = getImageBytes(s.serviceConfig.MySampleOption.OffImageLocation, buf)
			}
...
// command.
func (s *MySampleDriver) HandleWriteCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []sdkModels.CommandRequest,
	params []*sdkModels.CommandValue) error {
	var err error

	for i, r := range reqs {
		s.lc.Debugf("MySampleDriver.HandleWriteCommands: protocols: %v, resource: %v, parameters: %v, attributes: %v", protocols, reqs[i].DeviceResourceName, params[i], reqs[i].Attributes)
		switch r.DeviceResourceName {
		case "SwitchButton":
			if s.switchButton, err = params[i].BoolValue(); err != nil {
				err := fmt.Errorf("MySampleDriver.HandleWriteCommands; the data type of parameter should be Boolean, parameter: %s", params[0].String())
				return err
			}
		case "Xrotation":
			if s.xRotation, err = params[i].Int32Value(); err != nil {
				err := fmt.Errorf("MySampleDriver.HandleWriteCommands; the data type of parameter should be Int32, parameter: %s", params[i].String())
				return err
			}
		case "Yrotation":
		...
		case "StringArray":
			if s.stringArray, err = params[i].StringArrayValue(); err != nil {
				err := fmt.Errorf("MySampleDriver.HandleWriteCommands; the data type of parameter should be string array, parameter: %s", params[i].String())
				return err
			}
		case "Uint8Array":
		...
		case "Counter":
			if s.counter, err = params[i].ObjectValue(); err != nil {
				err := fmt.Errorf("MySampleDriver.HandleWriteCommands; the data type of parameter should be Object, parameter: %s", params[i].String())
				return err
			}
		}
	}
...
func (s *MySampleDriver) Stop(force bool) error {
	// Then Logging Client might not be initialized
	if s.lc != nil {
		s.lc.Debugf("MySampleDriver.Stop called: force=%v", force)
	}
	return nil
}

func (s *MySampleDriver) Start() error {
	return nil
}

// AddDevice is a callback function that is invoked
// when a new Device associated with this Device Service is added
func (s *MySampleDriver) AddDevice(deviceName string, protocols map[string]models.ProtocolProperties, adminState models.AdminState) error {
...
func (s *MySampleDriver) UpdateDevice(deviceName string, protocols map[string]models.ProtocolProperties, adminState models.AdminState) error {
...
func (s *MySampleDriver) RemoveDevice(deviceName string, protocols map[string]models.ProtocolProperties) error {
...
func (s *MySampleDriver) Discover() error {
	proto := make(map[string]models.ProtocolProperties)
	proto["other"] = map[string]any{"Address": "mysample02", "Port": 301}

	device2 := sdkModels.DiscoveredDevice{
		Name:        "MySample-Device02",
		Protocols:   proto,
	...
	proto = make(map[string]models.ProtocolProperties)
	proto["other"] = map[string]any{"Address": "mysample03", "Port": 399}

	device3 := sdkModels.DiscoveredDevice{
		Name:        "MySample-Device03",
		Protocols:   proto,
		Description: "found by discovery",
		Labels:      []string{"auto-discovery"},
	}
	...
	time.Sleep(time.Duration(s.serviceConfig.MySampleOption.Writable.DiscoverSleepDurationSecs) * time.Second)
	s.deviceCh <- res
	return nil
}

func (s *MySampleDriver) ValidateDevice(device models.Device) error {
